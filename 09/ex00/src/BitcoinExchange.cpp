...
namespace {
using svFloatPair = std::optional<std::pair<std::string_view, float>>;
using svSvPair = std::optional<std::pair<std::string_view, std::string_view>>;
...
svSvPair parseViews(const std::string& line, char delimiter) noexcept {
...
svFloatPair parseLine(const std::string& line, char delimiter) noexcept {
	svSvPair views{ parseViews(line, delimiter) };
...
template <std::invocable<svFloatPair, const std::string&> F>
void parseFile(const std::string& inFile, char delimiter, std::string_view headerValue, F func) {
	...
	while (std::getline(file, line)) {
		if (line.empty())
			continue;
		svFloatPair data{ parseLine(line, delimiter) };
		func(data, line);
	}
}
}
...
void BitcoinExchange::loadDatabase(const std::string& db) {
	auto insertData = [this](const svFloatPair& data, const std::string& line) {
...
void BitcoinExchange::processInput(const std::string& input) const {
	auto processData = [this](const svFloatPair& data, const std::string& line) {
		if (!data.has_value()) {
