...
enum class FormType { PRESIDENTIAL_PARDON, ROBOTOMY_REQUEST, SHRUBBERY_CREATION, UNDEFINED };

static FormType parseForm(const std::string& str) {
	static const std::string forms[]{ "presidential pardon", "robotomy request", "shrubbery creation" };
	for (size_t i{}; i < 3; ++i) {
		if (forms[i] == str)
...
AForm* Intern::makeForm(const std::string& str, std::string target) const {
	...
	switch (parseForm(str)) {
		case FormType::PRESIDENTIAL_PARDON:
			form = new PresidentialPardonForm{ std::move(target) };
			break;
		case FormType::ROBOTOMY_REQUEST:
			form = new RobotomyRequestForm{ std::move(target) };
			break;
		case FormType::SHRUBBERY_CREATION:
			form = new ShrubberyCreationForm{ std::move(target) };
			break;
